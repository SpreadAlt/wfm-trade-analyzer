...
type WfmTelemetry = {
  ...
  generatedAt: string
  configured: { intervalMs: number; targetRps: number; hardMinimumIntervalMs?: number; hardMaximumRps?: number; belowThreeRps?: boolean; hourlyIntervalMs?: number; hourlyMaxRps?: number; hourlyClient?: string; maxInFlight: number; warningDay: number; limitDay: number }
  current: { rps10s: number; rps60s: number; requests10s: number; requests60s: number; startedRps10s?: number; startedRps60s?: number; started10s?: number; started60s?: number; completedRps10s?: number; completedRps60s?: number; completed10s?: number; completed60s?: number; pending: number; pendingClients?: Record<string, number>; running: boolean; inFlight: number; inFlightClients?: Record<string, number>; coalescedKeys?: number; coalescedWaiters?: number; coalescedClients?: Record<string, number>; activeQueueClients?: string[]; startedByClient?: Record<string, { started10s: number; started60s: number; rps10s: number; rps60s: number }> }
  daily: {
    day: string
    requests: number
    started?: number
  ...
  upstream: {
    ...
    directWfmFetch?: boolean
    lastEgressRevision?: string | null
    ...
    rateLimitEventsStored?: number
    rateLimitEventsTotal?: number
    redirectEventsStored?: number
    redirectEventsTotal?: number
    redirectEvents?: Array<{ timestamp: string; status: number; client: string; requestId?: string; traceId?: string; hop?: number; from: string; location: string | null; allowedNext?: boolean; cfRay?: string | null; server?: string | null }>
    rateLimitEvents?: Array<{
      ...
      requests10s: number
      requests60s: number
      startedRps10s?: number
      startedRps60s?: number
      started10s?: number
      started60s?: number
      completedRps10s?: number
      completedRps60s?: number
      completed10s?: number
      completed60s?: number
      inFlight: number
      pending: number
  ...
  scheduling?: { mode: string; fairAcrossClients?: boolean; activeQueueClients?: string[]; perClientCaps?: Record<string, { intervalMs: number; maxRps: number; maxInFlight?: number; nextAllowedAt?: string | null }> }
}
...
export const DeveloperDashboard = ({ locale, onBack }: { locale: Locale; onBack: () => void }) => {
  ...
  return <main className="app-shell developer-shell">
    ...
    <section className="panel developer-process-panel" hidden={category !== 'overview'}>
      <header className="developer-section-heading"><div><span className="eyebrow">Runtime</span><h2>{ru ? 'Процессы в очереди' : 'Queued processes'}</h2><p>{ru ? 'Очереди Cloudflare и общий WFM Gateway. Gateway распределяет допуски между процессами по кругу. Общий потолок ≈3 req/s, а Hourly отдельно ограничен 1 req/s, поэтому его backlog не должен забирать всю полосу у Smart Buy, Axi или API.' : 'Cloudflare queues and the shared WFM Gateway. Gateway admissions are round-robin across processes. The global ceiling is ~3 req/s while Hourly is separately capped at 1 req/s, so its backlog cannot consume the whole lane needed by Smart Buy, Axi or API.'}</p></div><button type="button" className="secondary-action" onClick={() => void Promise.all([loadProcessQueues(), loadTelemetry()])}>{ru ? 'Обновить' : 'Refresh'}</button></header>
      ...
      <div className="developer-live-processes">
        ...
        <div><span>{ru ? 'Справедливый планировщик' : 'Fair scheduler'}</span><p>{telemetry?.scheduling?.mode === 'round-robin-by-client' ? `${ru ? 'по процессам' : 'by process'} · Hourly ≤ ${(telemetry.configured.hourlyMaxRps ?? 1).toFixed(2)} req/s${telemetry.current.activeQueueClients?.length ? ` · ${telemetry.current.activeQueueClients.join(' → ')}` : ''}` : (telemetry?.scheduling?.mode || '—')}</p></div>
    ...
    <section className="panel developer-telemetry-panel" hidden={category !== 'overview'}>
      <header className="developer-section-heading">
        <div><span className="eyebrow">Warframe Market gateway</span><h2>{ru ? 'Текущая скорость запросов' : 'Current request rate'}</h2><p>{ru ? 'Общая скорость API, Hourly, Smart Buy, Axi и сканера перепродажи: до ≈3 req/s. Hourly имеет отдельный потолок 1 req/s. Физические WFM-запросы идут через статический UpCloud egress.' : 'Combined rate for API, Hourly, Smart Buy, Axi and resale scanner: up to ~3 req/s. Hourly has a separate 1 req/s ceiling. Physical WFM requests use the static UpCloud egress.'}</p></div>
      ...
      <div className="developer-telemetry-summary">
        <div><span>{ru ? 'Запуски 10с' : 'Starts 10s'}</span><strong>{telemetry ? `${(telemetry.current.startedRps10s ?? telemetry.current.rps10s).toFixed(2)} req/s` : '—'}</strong><small>{telemetry ? `${telemetry.current.started10s ?? telemetry.current.requests10s} ${ru ? 'стартов' : 'started'} · ${telemetry.current.completed10s ?? telemetry.current.requests10s} ${ru ? 'ответов' : 'completed'}` : '—'}</small></div>
        <div><span>{ru ? 'Запуски 60с' : 'Starts 60s'}</span><strong>{telemetry ? `${(telemetry.current.startedRps60s ?? telemetry.current.rps60s).toFixed(2)} req/s` : '—'}</strong><small>{telemetry ? `${telemetry.current.started60s ?? telemetry.current.requests60s} ${ru ? 'стартов' : 'started'} · ${telemetry.current.completed60s ?? telemetry.current.requests60s} ${ru ? 'ответов' : 'completed'}` : '—'}</small></div>
        ...
        <div><span>{ru ? 'Сегодня' : 'Today'}</span><strong>{telemetry ? new Intl.NumberFormat(locale === 'ru' ? 'ru-RU' : 'en-US').format(telemetry.daily.started ?? telemetry.daily.requests) : '—'}</strong><small>{telemetry ? `${telemetry.daily.completed ?? telemetry.daily.requests} ${ru ? 'ответов' : 'completed'} · ${new Intl.NumberFormat(locale === 'ru' ? 'ru-RU' : 'en-US').format(telemetry.daily.remaining)} ${ru ? 'осталось' : 'remaining'}` : '—'}</small></div>
        <div><span>{ru ? 'Общий максимум' : 'Global ceiling'}</span><strong>{telemetry ? `${telemetry.configured.targetRps.toFixed(2)} req/s` : '—'}</strong><small>{telemetry ? `${telemetry.configured.intervalMs} ms · ≤ 3 req/s` : '—'}</small></div>
        <div><span>{ru ? 'Hourly максимум' : 'Hourly ceiling'}</span><strong>{telemetry ? `${(telemetry.current.startedByClient?.hourly?.rps60s ?? 0).toFixed(2)} req/s` : '—'}</strong><small>{telemetry ? `${ru ? 'лимит' : 'limit'} ${(telemetry.configured.hourlyMaxRps ?? 1).toFixed(2)} req/s · ${telemetry.configured.hourlyIntervalMs ?? 1000} ms` : '—'}</small></div>
        <div><span>{ru ? 'WFM egress' : 'WFM egress'}</span><strong>{telemetry?.upstream.egressConfigured ? (telemetry.upstream.egressIpv4 || 'Static IP') : '—'}</strong><small>{telemetry ? `${telemetry.upstream.transportMode || '—'}${telemetry.upstream.lastEgressRevision ? ` · ${telemetry.upstream.lastEgressRevision}` : ''}` : '—'}</small></div>
      ...
      <details className="developer-rate-limit-log">
        <summary className="developer-rate-limit-heading"><span>{ru ? 'Настоящие ответы WFM 429/509' : 'Upstream WFM 429/509 responses'} <b>{telemetry?.upstream.rateLimitEvents?.length ?? 0}{telemetry?.upstream.rateLimitEventsTotal != null ? ` / ${telemetry.upstream.rateLimitEventsTotal}` : ''}</b></span><small>{ru ? 'Показаны до 50 последних upstream-событий. Теперь скорость считается отдельно по фактическим запускам и завершённым ответам.' : 'Shows up to the last 50 upstream events. Rate is now measured separately for actual starts and completed responses.'}</small></summary>
        <div className="table-scroll"><table className="developer-table developer-rate-limit-table"><thead><tr><th>{ru ? 'Время / ID' : 'Time / ID'}</th><th>Endpoint</th><th>{ru ? 'Процесс' : 'Process'}</th><th>{ru ? 'Старт / ответ' : 'Start / complete'}</th><th>{ru ? 'Параллельно' : 'Concurrent'}</th><th>Retry-After</th><th>CF-Ray</th></tr></thead><tbody>
          {!telemetry?.upstream.rateLimitEvents?.length ? <tr><td colSpan={7} className="state-cell">{ru ? 'Зафиксированных upstream 429/509 пока нет.' : 'No upstream 429/509 events have been recorded yet.'}</td></tr> : telemetry.upstream.rateLimitEvents.map((event, index) => <tr key={`${event.timestamp}-${event.requestId || event.client}-${index}`}><td>{dateTime(event.timestamp, locale)}<small>HTTP {event.status}{event.throttleLevel ? ` · L${event.throttleLevel}` : ''}{event.sameThrottleWave ? ` · ${ru ? 'та же волна' : 'same wave'}` : ''}</small>{event.requestId ? <small><code title={event.traceId || event.requestId}>{event.requestId}</code></small> : null}</td><td><code>{event.endpoint}</code><small>{event.platform ? `${event.platform}${event.crossplay ? ` · crossplay=${event.crossplay}` : ''}` : event.route}{event.targetPath ? ` · ${event.targetPath}` : ''}{event.redirectsFollowed ? ` · redirects=${event.redirectsFollowed}` : ''}</small>{event.bodyPreview ? <details><summary>{ru ? 'Ответ WFM' : 'WFM response'}</summary><code title={event.bodyPreview}>{event.bodyPreview}</code></details> : null}</td><td>{event.client}</td><td>{(event.startedRps10s ?? event.rps10s).toFixed(2)} req/s<small>{event.started10s ?? event.requests10s}/{event.completed10s ?? event.requests10s} · 10s</small><small>{event.started60s ?? event.requests60s}/{event.completed60s ?? event.requests60s} · 60s</small></td><td>{event.inFlight}<small>{ru ? `ожидает: ${event.pending}` : `pending: ${event.pending}`}</small></td><td>{event.retryAfter || '—'}<small>{ru ? `применено ${event.appliedCooldownSeconds}с` : `applied ${event.appliedCooldownSeconds}s`}{event.retryAfterParsedSeconds != null ? ` · parsed ${event.retryAfterParsedSeconds}s` : ''}{!event.retryAfter && event.fallbackCooldownSeconds ? ` · fallback ${event.fallbackCooldownSeconds}s` : ''}</small></td><td><code>{event.cfRay || '—'}</code><small>{[event.server, event.egressIpv4 ? `egress=${event.egressIpv4}` : null, event.egressRevision || null, event.cfMitigated ? `mitigated=${event.cfMitigated}` : null, event.rateLimitRemaining != null ? `remaining=${event.rateLimitRemaining}` : null, event.rateLimitLimit != null ? `limit=${event.rateLimitLimit}` : null, event.rateLimitReset != null ? `reset=${event.rateLimitReset}` : null].filter(Boolean).join(' · ') || '—'}</small></td></tr>)}
        </tbody></table></div>
      </details>
      <details className="developer-rate-limit-log">
        <summary className="developer-rate-limit-heading"><span>{ru ? 'Редиректы WFM' : 'WFM redirects'} <b>{telemetry?.upstream.redirectEvents?.length ?? 0}{telemetry?.upstream.redirectEventsTotal != null ? ` / ${telemetry.upstream.redirectEventsTotal}` : ''}</b></span><small>{ru ? 'Gateway v1.4.5 отключает автоматическое следование fetch и считает каждый HTTP-hop отдельно. Если здесь 0 — один логический запрос не размножается редиректами.' : 'Gateway v1.4.5 disables automatic fetch redirect following and counts every HTTP hop separately. If this stays at 0, logical requests are not being multiplied by redirects.'}</small></summary>
        <div className="table-scroll"><table className="developer-table"><thead><tr><th>{ru ? 'Время / ID' : 'Time / ID'}</th><th>{ru ? 'Откуда' : 'From'}</th><th>Location</th><th>{ru ? 'Процесс' : 'Process'}</th><th>HTTP</th></tr></thead><tbody>
          {!telemetry?.upstream.redirectEvents?.length ? <tr><td colSpan={5} className="state-cell">{ru ? 'Редиректы от WFM не зафиксированы.' : 'No WFM redirects have been observed.'}</td></tr> : telemetry.upstream.redirectEvents.map((event, index) => <tr key={`${event.timestamp}-${event.requestId || index}`}><td>{dateTime(event.timestamp, locale)}{event.requestId ? <small><code title={event.traceId || event.requestId}>{event.requestId}</code></small> : null}</td><td><code>{event.from}</code></td><td><code>{event.location || '—'}</code><small>{event.allowedNext === false ? (ru ? 'заблокирован' : 'blocked') : ''}</small></td><td>{event.client}</td><td>{event.status}<small>{event.cfRay || event.server || '—'}</small></td></tr>)}
        </tbody></table></div>
      </details>
    </section>
