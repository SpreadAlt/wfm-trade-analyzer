  ...
  return <main className="app-shell developer-shell">
    ...
    <section className="panel developer-telemetry-panel" hidden={category !== 'overview'}>
      <header className="developer-section-heading">
        <div><span className="eyebrow">Warframe Market gateway</span><h2>{ru ? 'Текущая скорость запросов' : 'Current request rate'}</h2><p>{ru ? 'Общая скорость API, Hourly, Smart Buy, Axi и сканера перепродажи. Физические WFM-запросы идут через статический UpCloud egress. Данные обновляются каждые 10 секунд.' : 'Combined rate for API, Hourly, Smart Buy, Axi and resale scanner. Physical WFM requests use the static UpCloud egress. Refreshes every 10 seconds.'}</p></div>
      ...
      <details className="developer-rate-limit-log">
        <summary className="developer-rate-limit-heading"><span>{ru ? 'Редиректы WFM' : 'WFM redirects'} <b>{telemetry?.upstream.redirectEvents?.length ?? 0}{telemetry?.upstream.redirectEventsTotal != null ? ` / ${telemetry.upstream.redirectEventsTotal}` : ''}</b></span><small>{ru ? 'Gateway v1.4.4 отключает автоматическое следование fetch и считает каждый HTTP-hop отдельно. Если здесь 0 — один логический запрос не размножается редиректами.' : 'Gateway v1.4.4 disables automatic fetch redirect following and counts every HTTP hop separately. If this stays at 0, logical requests are not being multiplied by redirects.'}</small></summary>
